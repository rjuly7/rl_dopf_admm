...
if casename == "case118_3"
    pqs_l = [100, 100, 100, 100, 100, 100, 100, 100, 150, 200, 250, 300, 350, 375]
    pqs_u = [7000, 6000, 5000, 4000, 3600, 3200, 2800, 2000, 1200, 900, 700, 600, 500, 450]

    vts_l = [200, 400, 500, 800, 1200, 1400, 1600, 2000, 2400, 2800, 3200, 3600, 3800, 3900]
    vts_u = [14000, 13000, 12000, 9000, 7200, 7000, 6500, 6000, 5600, 5200, 4800, 4400, 4200, 4100]
...
n_iters_mean = vcat([99, 78, 70.9],n_iters_mean)
...
plot(spread, pqs_l, fillrange = pqs_u, fillalpha = 0.35, label = "PQ bounds",xlabel="Length of search space", ylabel="Search values")
plot!(spread, vts_l, fillrange = vts_u, fillalpha = 0.35, label = "VT bounds")
...
plot(spread, (baseline_iter .- n_iters_mean)./baseline_iter*100, linewidth=2,label="",xlabel="Length of search space",ylabel="Decrease in number of iterations (%)")
yticks!(0:10:100, ["$tick%" for tick in 0:10:60])
savefig("data/figs/$casename"*"_percent_decrease.png")
...
n_iters_mean = [165,124,105,98,101,97,188]
baseline_iter = 178
plot(spread,n_iters_mean,ylim=(0,baseline_iter + 10),linewidth=2,label="LinUCB",xlabel="Length of search space",ylabel="Iterations to converge")
plot!(spread,baseline_iter*ones(length(spread)),linewidth=2,label="Baseline")
savefig("data/figs/$casename"*"_iters_vs_space.png")
