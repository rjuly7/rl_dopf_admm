...
function get_hyperparameter_configuration(data_area,pq_bounds,vt_bounds,rng)
    ...
    for area_id in keys(data_area)
        ...
        for neighbor in keys(data["alpha"])
            for opf_var in keys(data["alpha"][neighbor])
                for i in keys(data["alpha"][neighbor][opf_var])
                    if opf_var == "pt" || opf_var == "pf" || opf_var == "qt" || opf_var == "qf"
                        aa = rand(rng)*(pq_bounds[2]-pq_bounds[1])+pq_bounds[1] 
                        alpha_config[area_id][neighbor][opf_var][i] = aa 
                        push!(alpha_vector, aa)
                        push!(is_pq,true)
                    else
                        aa =  rand(rng)*(vt_bounds[2]-vt_bounds[1])+vt_bounds[1] 
                        alpha_config[area_id][neighbor][opf_var][i] = aa
                        push!(alpha_vector, aa)
                        push!(is_pq,false)
                    end
    ...
    return alpha_config, alpha_vector, is_pq 
end
...
function run_some_iterations(data_area::Dict{Int,Any}, dopf_method::Module, model_type::DataType, optimizer, iteration::Int, alpha_config::Vector{AbstractFloat}, iters_to_run::Int, rng)
...
function run_to_end(data_area::Dict{Int,Any}, alpha_config;
...
function run_to_end(data_area::Dict{Int,Any}, Q, pq_action_set, vt_action_set, n_history, update_alpha_freq;
        dopf_method=adaptive_admm_methods, model_type=ACPPowerModel, optimizer=Ipopt.Optimizer, max_iteration=1000)
    flag_convergence = false
    #We want to store the 2-norm of primal and dual residuals
    #at each iteration
    ...
    agent_residual_data = Dict(i => Dict("primal" => [20*ones(n_history,Float64)], "dual" => [30000*ones(n_history,Float64)]) for i in areas_id)
    iteration = 1 
    reward_residual_data = Dict("primal" => [], "dual" => []) 
    ...
    while iteration < max_iteration && !flag_convergence
        ...
        if mod(iteration-1,update_alpha_freq) == 0 
            for area in areas_id 
                state = vcat(tanh_norm(agent_residual_data[area]["primal"][end-n_history+1:end]),tanh_norm(agent_residual_data[area]["dual"][end-n_history+1:end]))
